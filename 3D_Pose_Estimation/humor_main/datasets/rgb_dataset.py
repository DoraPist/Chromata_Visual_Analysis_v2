...
from humor.fitting.fitting_utils import read_keypoints, load_planercnn_res
...
class RGBVideoDataset(Dataset):
    ...
    def __init__(self, joints2d_path,   # Dora's change
             cam_mat,
             seq_len=None,
             overlap_len=None,
             img_path=None,
        ...
        - person_id : identifies selected person for 3d pose estimation when we use as input a video with more than 1 individuals (Dora's addition)
        '''
        ...
        self.person_id = person_id   # Dora's addition
    ...
    def load_data(self):
        ...
        keyp_paths = sorted(glob.glob(self.joints2d_path + '/*_keypoints.json'))
        print("Keyp_paths: ", keyp_paths)
        frame_names = [f.split('\\')[-1].split('_')[0] for f in keyp_paths]
        # for f in keyp_paths:
        #     # temp = f.split('\\')[-1].split('_')[0]
        #     frame_names = [f.split('\\')[-1].split('_')[0]]
        ...
        if self.seq_len is not None and self.overlap_len is not None:
            ...
            r = self.seq_len * num_seqs - self.overlap_len * (
                        num_seqs - 1) - num_frames  # number of extra frames we cover
            ...
            new_cov = self.seq_len * num_seqs - self.overlap_len * (
                        num_seqs - 1)  # now compute how many frames are still left to account for
            ...
            print('Splitting into subsequences of length %d frames overlapping by %d...' % (
            self.seq_len, self.overlap_len))
        else:
        ...
        if self.img_path is not None:
            ...
            img_paths = [self.img_path + '/' + img_fn
        ...
        if self.masks_path is not None:
            ...
            mask_paths = [self.masks_path + '/' + f + '.png' for f in frame_names]
        ...
        for seq_idx in range(num_seqs):
            ...
            keyp_frames = [read_keypoints(f, self.person_id) for f in keyp_paths[sidx:eidx]]
    ...
    def __getitem__(self, idx):
        ...
        if self.mask_joints:
            ...
            # DORA TODO Correct masks path!!!!
            print("cur_mask_paths: ", cur_mask_paths[0])
