...
# Globals
male_model_path = 'Mixamo_Characters/remy.fbx'   # works fine in terms of skeleton!!! (mixamorig)
female_model_path = 'Mixamo_Characters/remy.fbx'
...
def process_pose(current_frame, pose, trans, pelvis_position):
    ...
    # elif pose.shape[0] == 74:
    #     rod_rots = pose[0:72].reshape(24, 3)
    else:
    ...
    for index, mat_rot in enumerate(mat_rots, 0):
        ...
        quat_x_90_cw = Quaternion((1.0, 0.0, 0.0), radians(0))
        quat_y_180cw = Quaternion((0.0, 1.0, 0.0), radians(180))
        quat_z_90_cw = Quaternion((0.0, 0.0, 1.0), radians(180))
        ...
        if index == 0:
            ...
            bone.rotation_quaternion = (quat_x_90_cw @ quat_y_180cw @ quat_z_90_cw) @ bone_rotation
        else:
...
def process_poses(
    ...
    # bpy.ops.graph.smooth(poses)    #Dora
    ...
    if gender == 'female':
        ...
        for k,v in bone_name_from_index.items():
            bone_name_from_index[k] = 'mixamorig2:' + v
    elif gender == 'male':
        model_path = male_model_path
        for k,v in bone_name_from_index.items():
            bone_name_from_index[k] = 'mixamorig:' + v
    else:
    ...
    # dora_tmp = bpy.data.armatures[0].edit_bones[bone_name_from_index[0]]
...
def export_animated_mesh(output_path):
    ...
    bpy.data.objects['Armature'].children[1].select_set(True)
    bpy.data.objects['Armature'].children[2].select_set(True)
    bpy.data.objects['Armature'].children[3].select_set(True)
    bpy.data.objects['Armature'].children[4].select_set(True)
    bpy.data.objects['Armature'].children[5].select_set(True)
    bpy.data.objects['Armature'].children[6].select_set(True)

    # # Add also materials (Dora)
    ...
    armature_check = bpy.data.objects['Armature']
