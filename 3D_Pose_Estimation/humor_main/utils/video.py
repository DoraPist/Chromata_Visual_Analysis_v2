...
def video_to_images(vid_file, img_folder=None, return_info=False, fps=30):
    ...
    print(osp.join(img_folder, '000001.png'))
...
def run_openpose(openpose_path, img_dir, out_dir, video_out=None, img_out=None):
    ...
    # print('./build/x64/Release/OpenPoseDemo.exe', \
    #             '--image_dir', img_dir, '--write_json', out_dir, \
    #             '--display', '0', '--model_pose', SKELETON, '--number_people_max', '1')

    # run_cmds = ['build/x64/Release/OpenPoseDemo.exe', \
    #             '--image_dir', img_dir, '--write_json', out_dir, \
    #             '--display', '1', '--model_pose', SKELETON, '--number_people_max', '1']

    # run_cmds = 'build/x64/Release/OpenPoseDemo.exe ' + '--image_dir ' + img_dir + ' --write_json ' + out_dir + ' --display ' + '1' + ' --model_pose '+ SKELETON + ' --number_people_max ' + '1'

    run_cmds = "build/x64/Release/OpenPoseDemo.exe --image_dir " + img_dir + " --write_json " + out_dir + \
               " --display 0 --model_pose " + SKELETON + " --number_people_max 1 --write_images " + img_out + \
               " --write_video " + video_out + " --write_video_fps 30"

    # run_cmds = "build/x64/Release/OpenPoseDemo.exe --image_dir " + img_dir + " --write_json " + out_dir + \
    #             " --display 0 --model_pose " + SKELETON + " --number_people_max 2 " + "--write_images " + img_out + \
    #             " --write_video " + video_out + " --write_video_fps 30 --tracking 5"
...
def run_alphapose(alphapose_path, img_dir, out_dir, video_out=None, img_out=None):
    '''
    Runs AlphaPose for 2D joint detection on the images in img_dir.
    '''
    # make all paths absolute to call OP
    alphapose_path = make_absolute([alphapose_path])[0]
    img_dir = make_absolute([img_dir])[0]
    out_dir = make_absolute([out_dir])[0]
    if video_out is not None:
        video_out = make_absolute([video_out])[0]
    ...
    # run open pose
    # must change to openpose/alphapose dir path to run properly
    og_cwd = os.getcwd()
    ...
    #             " --display 0 --model_pose " + SKELETON + " --number_people_max 1 --write_images " + img_out + \
    #             " --write_video " + video_out + " --write_video_fps 30"
    # run_cmds = "python scripts/demo_inference.py --detector yolox-x --cfg configs/coco/resnet/256x192_res50_lr1e-3_1x.yaml " \
    #            "--checkpoint pretrained_models/fast_res50_256x192.pth --indir " + img_dir + " --pose_track --format open --outdir " + out_dir + " --save_img --showbox"
    run_cmds = "python scripts/demo_inference.py --detector yolox-x --cfg configs/halpe_26/resnet/256x192_res50_lr1e-3_1x.yaml " \
               "--checkpoint pretrained_models/halpe26_fast_res50_256x192.pth --indir " + img_dir + " --pose_track --format open " \
               "--outdir " + out_dir + " --save_img --showbox --vis_fast --min_box_area 100"
    ...
    # Write video from images with bboxes
    img_folder = out_dir + "/op_frames/%06d.png"
    # command = "ffmpeg -y -threads 16 -i " + img_folder + " -profile:v baseline -level 3.0 -c:v libx264 -pix_fmt yuv420p -an -v error " + video_out
    # command = "ffmpeg -i " + img_folder + " -vcodec libx264 " + video_out

    # command = 'ffmpeg -y -threads 16 -i ' + img_folder + ' -profile:v baseline -level 3.0 -c:v libx264 -pix_fmt yuv420p -an -v error ' + video_out
    command = 'ffmpeg -y -threads 16 -i ' + img_folder + ' -profile:v baseline -level 3.0 -codec:v libx264 -pix_fmt yuv420p -an -v error ' + video_out

    #     [
    #     'ffmpeg', '-y', '-threads', '16', '-i', f'{img_folder}/%06d.png', '-profile:v', 'baseline',
    #     '-level', '3.0', '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-an', '-v', 'error', video_out,
    # ]

    # os.system("ffmpeg -i " + keyframes_path + "pre_video_shot_" + str(
    #     i) + ".mp4" + " -vcodec libx264 " + keyframes_path + "video_shot_" + str(i) + ".mp4")
...
def run_deeplab_v3(img_dir, img_shape, out_dir, batch_size=16, img_extn='png'):
    ...
    all_img_paths = sorted(glob.glob(img_path + '/*.' + img_extn))
    # img_names = ['.'.join(f.split('/')[-1].split('.')[:-1]) for f in all_img_paths]
    # for f in all_img_paths:
    #     print(f.split('\\')[-1].split('.')[0])
    # img_names = ['./' + f.split('/')[-1].split('.')[:-1] for f in all_img_paths]
    img_names = [f.split('/')[-1].split('.')[0] for f in all_img_paths]
