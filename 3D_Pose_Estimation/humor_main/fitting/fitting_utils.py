...
from humor.utils.transforms import rotation_matrix_to_angle_axis, batch_rodrigues, convert_to_rotmat
from humor.utils.logging import mkdir, Logger
...
def read_keypoints(keypoint_fn, person_id): # Dora's change for multiperson 3D pose estimation
    '''
    Reads body keypoint data of person with selected person_id.
    '''
    with open(keypoint_fn) as keypoint_file:
        data = json.load(keypoint_file)

        # Get right keypoints   # Dora's change for multiperson 3D pose estimation
        index_person = 0
        for item in data['people']:
            if not item["person_id"] == person_id:
    ...
    try:
        ...
        print("Index person in keypoints json: ", index_person)
        print(keypoint_fn)
        person_data = data['people'][index_person]
        body_keypoints = np.array(person_data['pose_keypoints_2d'],
                                  dtype=np.float)
        body_keypoints = body_keypoints.reshape([-1, 3])
    except:
        print("No person_id in frame ", keypoint_fn)
        body_keypoints = []
...
def save_rgb_stitched_result(seq_intervals, all_res_out_paths, res_out_path, device,
    ...
    for res_idx, res_dir in enumerate(all_res_dirs):
        ...
        if concat_ground_planes is None:
            ...
            concat_ground_planes = torch.cat(
                [concat_ground_planes, torch.Tensor(cur_stage3_res['floor_plane']).to(device).reshape((1, -1))], dim=0)
        cur_stage3_res = {k: v for k, v in cur_stage3_res.items() if
                          k in ['betas', 'trans', 'root_orient', 'pose_body']}
    ...
    src_meta_path = all_res_dirs[0] + '/meta.txt'
    # shutil.copyfile(src_meta_path, os.path.join(final_res_out_path, 'meta.txt'))
    shutil.copyfile(src_meta_path, final_res_out_path + '/meta.txt')

    #  gt results (cam matx)
    # np.savez(os.path.join(final_res_out_path, 'gt_results.npz'), cam_mtx=gt_cam_mtx)
    np.savez(final_res_out_path + '/gt_results.npz', cam_mtx=gt_cam_mtx)
    ...
    np.savez(final_res_out_path + '/observations.npz', joints2d=concat_joints2d, img_paths=concat_img_paths)

    #  save the actual results npz for viz later
    # concat_res_out_path = os.path.join(final_res_out_path, 'stage3_results.npz')
    concat_res_out_path = final_res_out_path + '/stage3_results.npz'
    ...
    res_floor_plane = concat_ground_planes[
        0].clone().detach().cpu().numpy()  # NOTE: saves estimate from first subsequence
    ...
    input_data_dict = {kb: vb.unsqueeze(0) for kb, vb in concat_cam_res.items() if
                       kb in ['trans', 'root_orient', 'pose_body', 'betas']}
    ...
    concat_prior_res_out_path = final_res_out_path + '/stage3_results_prior.npz'
...
def apply_cam2prior(data_dict, R, t, root_height, body_pose, betas, key_frame_idx, body_model, inverse=False):
    ...
    if 'root_orient' in data_dict:
        ...
        prior_root_orient = rotation_matrix_to_angle_axis(prior_root_orient_mat.reshape((B * T, 3, 3))).reshape(
            (B, T, 3))
...
OP_EDGE_LIST = [[1, 8], [1, 2], [1, 5], [2, 3], [3, 4], [5, 6], [6, 7], [8, 9], [9, 10], [10, 11], [8, 12], [12, 13],
                [13, 14], [1, 0], [0, 15], [15, 17], [0, 16], [16, 18], [14, 19], [19, 20], [14, 21], [11, 22],
                [22, 23], [11, 24]]
...
def expid2model(expr_dir):
    ...
    # os.chdir("/home/tpistola/Projects/Chromata/Services_v1/Pose-Estimation_v2/")
    os.chdir("C:/Chromata/Services_v1/Pose-Estimation_v2/")
    print("Current dir - new: ", os.getcwd())
    ...
    best_model_fname = sorted(glob.glob(expr_dir + '/snapshots/' + '*.pt'), key=os.path.getmtime)[-1]
    ...
    default_ps_fname = glob.glob(expr_dir + '/*.ini')[0]
...
def load_vposer(expr_dir, vp_model='snapshot'):
    ...
    if vp_model == 'snapshot':
        ...
        vposer_path = sorted(glob.glob(expr_dir + '/vposer_*.py'), key=os.path.getmtime)[-1]
