...
def parse_args(argv):
    ...
    # --results
    # ../../ out / rgb_demo_use_split / results_out
    # --out
    # ../..out / rgb_demo_use_split / viz_out
    # --viz - final - only
...
def create_pkl(results, out, person_id):
    ...
    # visualization options
    # parser.add_argument('--viz-final-only', dest='viz_final_only', action='store_true',
    #                     help="If given only visualize the final full sequence result and not the subsequences.")
    # parser.set_defaults(viz_final_only=False)
    viz_final_only = False

    # parser.add_argument('--viz-stages', dest='viz_stages', action='store_true',
    #                     help="If given, visualizes intermediate optimization stages and comparison to final pred.")
    # parser.set_defaults(viz_stages=False)

    viz_stages = False
    # parser.add_argument('--viz-prior-frame', dest='viz_prior_frame', action='store_true',
    #                     help="If given, also visualizes results in the HuMoR canonical coordinate frame.")
    # parser.set_defaults(viz_prior_frame=False)
    # parser.add_argument('--viz-obs-2d', dest='viz_obs_2d', action='store_true',
    #                     help="If given, visualizes 2D joint observations on top of og video")
    # parser.set_defaults(viz_obs_2d=False)
    # parser.add_argument('--viz-no-render-cam-body', dest='viz_render_cam_body', action='store_false',
    #                     help="If given, does not render body mesh from camera view")
    # parser.set_defaults(viz_render_cam_body=True)
    # parser.add_argument('--viz-pred-floor', dest='viz_pred_floor', action='store_true',
    #                     help="Render the predicted floor from the camera view.")
    # parser.set_defaults(viz_pred_floor=False)
    # parser.add_argument('--viz-contacts', dest='viz_contacts', action='store_true',
    #                     help="Render predicted contacts on the joints")
    # parser.set_defaults(viz_contacts=False)
    # parser.add_argument('--viz-wireframe', dest='viz_wireframe', action='store_true',
    #                     help="Render body and floor in wireframe")
    # parser.set_defaults(viz_wireframe=False)
    # parser.add_argument('--viz-bodies-static', type=int, default=None,
    #                     help="If given, renders all body predictions at once at this given frame interval interval.")
    # parser.add_argument('--viz-no-bg', dest='viz_bg', action='store_false',
    #                     help="If given will not overlay the rendering on top of OG video.")
    # parser.set_defaults(viz_bg=True)

    viz_prior_frame = False
    ...
    # parser.add_argument('--flip-img', dest='flip_img', action='store_true',
    #                     help="Flips the loaded image about y-axis. This is useful for PROX result.")
    # parser.set_defaults(flip_img=False)

    D_IMW, D_IMH = 1280, 720
    ...
    # os.chdir('humor/fitting/')
    print("Current directory after change: ", os.getcwd())
    # the_list = os.listdir(args.results)

    all_result_dirs = [results + '/person_' + str(person_id) + '/' + f for f in sorted(os.listdir(results + '/person_' + str(person_id))) if f[0] != '.']
    ...
    for residx, result_dir in enumerate(all_result_dirs):  #This was the dafault code - It runs for all person_ids
        ...
        cur_viz_stages = viz_stages and not is_final_res
        cur_stages_res = None
        if cur_viz_stages:
            cur_stages_res = dict()
        ...
        if viz_prior_frame and cur_viz_stages:
        ...
        data = np.load("../../out/rgb_demo_use_split/results_out/person_" + str(person_id) + "/final_results/stage3_results_prior.npz")
        trans = data.f.trans
