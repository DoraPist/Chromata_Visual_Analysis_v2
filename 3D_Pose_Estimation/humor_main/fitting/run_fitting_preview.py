...
from humor.datasets.prox_dataset import ProxDataset
from humor.datasets.imapper_dataset import iMapperDataset
from humor.datasets.rgb_dataset import RGBVideoDataset
from humor.utils.torch import load_state
from humor.utils.logging import mkdir
from humor.fitting.config import parse_args
from humor.fitting.fitting_utils import NSTAGES, DEFAULT_FOCAL_LEN, load_vposer, save_optim_result, save_rgb_stitched_result
from humor.fitting.motion_optimizer import MotionOptimizer
from humor.utils.video import video_to_images, run_openpose, run_deeplab_v3, run_alphapose

from humor.body_model.body_model import BodyModel
from humor.body_model.utils import SMPLX_PATH, SMPLH_PATH
...
def main(args):
    ...
    person_id_list = [1]  # Dora's addition (in order to have a list of people we want to get their 3d poses)

    for person_id in person_id_list:
        print("alphapose person id: ", person_id)
        if person_id == person_id_list[0]:
            res_out_path = res_out_path + "person_" + str(person_id) + "/"
        else:
            res_out_path = res_out_path + "../person_" + str(person_id) + "/"

        # os.makedirs(res_out_path, exist_ok=True)   # Create a separate folder for the results of each person_id
        print("CHECK THIS: res_out_path", res_out_path)
        ...
        if args.data_type == 'AMASS':
            ...
            if not os.path.exists(video_preprocess_path) or (not use_custom_keypts and not os.path.exists(op_out_path)):
                ...
                if not use_custom_keypts:
                    # OpenPose on images
                    print("video_preprocess_path: ", video_preprocess_path)
                    op_frames_out = os.path.join(video_preprocess_path, 'op_frames')
                    # run_openpose(args.openpose, img_folder,
                    #             op_out_path,
                    #             img_out=op_frames_out,
                    #             # video_out=os.path.join(video_preprocess_path, 'op_keypoints_overlay.avi'))
                    #             video_out=video_preprocess_path + '/op_keypoints_overlay.avi')
                    alphapose_path = args.alphapose
                    run_alphapose(alphapose_path, img_folder,
                                 video_preprocess_path,
                                 img_out=op_frames_out,
                                 # video_out=os.path.join(video_preprocess_path, 'op_keypoints_overlay.avi'))
                                 video_out=video_preprocess_path + '/op_keypoints_overlay.mp4')
                ...
                import cv2
                print("TEST-img_folder: ", img_folder)
                # img_shape = cv2.imread(os.path.join(img_folder, '000001.png')).shape
                img_shape = cv2.imread(img_folder + '/000001.png').shape
        ...
        #
        #     temp = args.data_path.split('/')[-1].split('.')[0]
        #
        #     vid_name = args.data_path.split('/')[-1].split('.')[0]
        ...
        #     #                         )
        #
        #     # person_id = 1  # Dora's addition (this corresponts to person id extracted from AplhaPose)
        #
        #     dataset = RGBVideoDataset(op_out_path,     # Dora's change
        #                               cam_mat,
        #                               seq_len=args.rgb_seq_len,
        #                               overlap_len=args.rgb_overlap_len,
        #                               img_path=img_folder,
        ...
        #     gmm_path = args.init_motion_prior + '/prior_gmm.npz'
        ...
        #             cur_res_out_path = res_out_path + seq_name
        ...
        #
        #     # replace any '\' with '/'
        #     body_model_path.replace('\\', '/')
        #
        #     fit_gender = body_model_path.split('/')[-2]
        #
        #     print("fit_gender: ", fit_gender)
        #
        ...
        #                 gt_body_path = SMPLH_PATH + '/%s/model.npz' % (cur_gender)
        ...
        #             cur_meta_path = cur_res_out_path + '/meta.txt'
