...
def parse_args(argv):
    ...
    parser.add_argument('--data-type', type=str, default="RGB", required=False, choices=['AMASS', 'PROX-RGB', 'PROX-RGBD', 'iMapper-RGB', 'RGB'], help='The type of data we are fitting to.')
    parser.add_argument('--data-fps', type=int, default=30, help='Sampling rate of the data.')
    parser.add_argument('--batch-size', type=int, default=2, help='Number of sequences to batch together for fitting to data.')
    ...
    # parser.add_argument('--rgb-seq-len', type=int, default=60, help='If none, fits the whole video at once. If given, is the max number of frames to use when splitting the video into subseqeunces for fitting.')
    ...
    # parser.add_argument('--rgb-overlap-len', type=int, default=10, help='If None, fits the whole video at once. If given, is the minimum number of frames to overlap subsequences extracted from the given video. These overlapped frames are used in a consistency energy.')
    parser.add_argument('--rgb-overlap-len', type=int, default=None, help='If None, fits the whole video at once. If given, is the minimum number of frames to overlap subsequences extracted from the given video. These overlapped frames are used in a consistency energy.')
    ...
    parser.add_argument('--rgb-overlap-consist-weight', type=float,nargs=NSTAGES, default=[200.0, 200.0, 200.0], help='Enforces consistency between overlapping subsequences within a batch in terms of the ground plane, shape params, and joint positions.')

    # PROX + iMapper + RGB options
    parser.add_argument('--mask-joints2d', default=True, dest='mask_joints2d', action='store_true', help="If true, masks the 2d joints based on the person segmentation occlusion mask.")
    ...
    parser.add_argument('--joint3d-smooth-weight', type=float,nargs=NSTAGES, default=[100.0, 100.0, 0.0], help='L2 loss on 3D joints differences')
    ...
    parser.add_argument('--joint2d-weight', type=float,nargs=NSTAGES, default=[0.001, 0.001, 0.001], help='L2 loss on 2D reprojection')
    parser.add_argument('--pose-prior-weight', type=float,nargs=NSTAGES, default=[0.04, 0.04, 0.0], help='likelihood under pose prior')
    parser.add_argument('--shape-prior-weight', type=float,nargs=NSTAGES, default=[0.05, 0.05, 0.05], help='likelihood under shape prior')
    parser.add_argument('--motion-prior-weight', type=float,nargs=NSTAGES, default=[0.0, 0.0, 0.075], help='likelihood under motion prior')
    parser.add_argument('--init-motion-prior-weight', type=float,nargs=NSTAGES, default=[0.0, 0.0, 0.075], help='likelihood under init state prior')
    parser.add_argument('--joint-consistency-weight', type=float,nargs=NSTAGES, default=[0.0, 0.0, 100.0], help='L2 difference between SMPL and motion prior joints')
    parser.add_argument('--bone-length-weight', type=float,nargs=NSTAGES, default=[0.0, 0.0, 2000.0], help='L2 difference between bone lengths of motion prior joints at consecutive frames.')
    parser.add_argument('--contact-vel-weight', type=float,nargs=NSTAGES, default=[0.0, 0.0, 100.0], help='Predicted contacting joints have 0 velocity when in contact')
    parser.add_argument('--contact-height-weight', type=float,nargs=NSTAGES, default=[0.0, 0.0, 10.0], help='Predicted contacting joints are at the height of the floor')
    parser.add_argument('--floor-reg-weight', type=float,nargs=NSTAGES, default=[0.0, 0.0, 0.167], help='L2 Regularization that pushes floor to stay close to the initialization.')
    # loss options
    ...
    # smpl model path
    parser.add_argument('--smpl', type=str, default='/humor_main/body_models/smplh/neutral/model.npz', help='Path to SMPL model to use for optimization. Currently only SMPL+H is supported.')
    parser.add_argument('--gt-body-type', type=str, default='smplh', choices=['smplh'], help='Which body model to load in for GT data')
    parser.add_argument('--vposer', type=str, default='humor_main/body_models/vposer_v1_0', help='Path to VPoser checkpoint.')
    # parser.add_argument('--openpose', type=str, default='./external/openpose', help='Path to OpenPose installation.')
    parser.add_argument('--alphapose', type=str, default='humor_main/AlphaPose', help='Path to OpenPose installation.')

    # motion prior weights and model information
    parser.add_argument('--humor', type=str, default="humor_main/checkpoints/humor/best_model.pth", help='Path to HuMoR weights to use as the motion prior.')
    ...
    parser.add_argument('--humor-steps-in', type=List[int], default=1, help='Number of input timesteps the prior expects.')

    # init motion state prior information
    parser.add_argument('--init-motion-prior', type=str, default='humor_main/checkpoints/init_state_prior_gmm', help='Path to parameters of a GMM to use as the prior for initial motion state.')

    # optimization options
    parser.add_argument('--lr', type=float, default=1.0, help='step size during optimization')
    parser.add_argument('--num-iters', type=int, nargs=NSTAGES, default=[1, 1, 1], help='The number of optimization iterations at each stage (3 stages total)') #30, 80, 70
    ...
    parser.add_argument('--out', type=str, default="", help='Output path to save fitting results/visualizations to.')

    # Dora's additions
    parser.add_argument('--gender', type=str, default="neutral", help='Gender of avatar.')
    parser.add_argument('--person_id', type=int, default=[], help='Id of person/dancer to use for pose estimation.')
    ...
    parser.set_defaults(save_stages_results=True)
